...
function Student(props){
    ...
    return(
        <div>
            ...
            <p>This student is a member of: <Link to={`/campuses/${props.campus.id}`}>{props.campus.name}</Link></p>
        </div>
    )
}
...
const mapState = function (state, ownProps) {
    ...
    return {
        ...
        campus: state.campuses.find(campus => campus.id === foundStudent.campusId) || {id: 0}
    };
}
